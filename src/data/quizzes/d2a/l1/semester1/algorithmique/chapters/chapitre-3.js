export const chapter3 = {
  id: "chapter-3",
  title: "Boucles",
  questions: [
    {
      id: 1,
      question: "Rôle des structures itératives",
      options: [
        "Stocker des données",
        "Répéter des actions",
        "Afficher uniquement",
        "Déclarer des variables",
      ],
      answer: 1,
    },
    {
      id: 2,
      question: "Nombre de répétitions connu",
      options: ["Tant Que", "Répéter", "Pour", "Si"],
      answer: 2,
    },
    {
      id: 3,
      question: "Structure pour un contrôle de saisie ",
      options: ["Pour", "Répéter Jusqu'à", "Si", "Cas"],
      answer: 1,
    },
    {
      id: 4,
      question: "S'exécute au moins une fois ",
      options: ["Pour", "Tant Que", "Répéter Jusqu'à", "Si"],
      answer: 2,
    },
    {
      id: 5,
      question: "Syntaxe correcte d'une boucle Pour ",
      options: [
        "Pour i = 1 à 10 faire",
        "Pour i <= 10",
        "Tant que i <= 10",
        "Répéter i",
      ],
      answer: 0,
    },
    {
      id: 6,
      question: "Nombre d'itérations : Pour i:=1 à 10 ",
      options: ["9 fois", "10 fois", "11 fois", "Indéterminé"],
      answer: 1,
    },
    {
      id: 7,
      question: "Boucle Pour ",
      options: ["while", "repeat", "for", "loop"],
      answer: 2,
    },
    {
      id: 8,
      question: "Compter de 10 vers 1 ",
      options: ["for i:=10 to 1", "for i:=10 downTo 1", "while i>0", "repeat"],
      answer: 1,
    },
    {
      id: 9,
      question: "Dans un Pour, le compteur est",
      options: ["Modifié automatiquement", "Fixe", "Supprimé", "Aléatoire"],
      answer: 0,
    },
    {
      id: 10,
      question: "Modifier le compteur manuellement dans un For",
      options: ["Recommandé", "Déconseillé", "Obligatoire", "Sans effet"],
      answer: 1,
    },
    {
      id: 11,
      question: "La boucle Tant Que s'exécute si ",
      options: [
        "Condition vraie",
        "Condition fausse",
        "Compteur existe",
        "Programme démarre",
      ],
      answer: 0,
    },
    {
      id: 12,
      question: "Boucle Tant Que ",
      options: ["repeat", "for", "while", "loop"],
      answer: 2,
    },
    {
      id: 13,
      question: "Risque avec une boucle While ",
      options: [
        "Erreur de compilation",
        "Boucle infinie",
        "Variables effacées",
        "Aucun",
      ],
      answer: 1,
    },
    {
      id: 14,
      question: "Dans un While, il faut généralement",
      options: [
        "Modifier la condition",
        "Créer une constante",
        "Créer une fonction",
        "Afficher un message",
      ],
      answer: 0,
    },
    {
      id: 15,
      question: "Dans Répéter Jusqu'à, le test a lieu ",
      options: [
        "Avant l'exécution",
        "Après l'exécution",
        "Jamais",
        "Deux fois",
      ],
      answer: 1,
    },
    {
      id: 16,
      question: "Répéter Jusqu'à ",
      options: ["while", "for", "repeat ... until", "case"],
      answer: 2,
    },
    {
      id: 17,
      question: "Structure idéale pour yⁿ ",
      options: ["Pour", "Si", "Cas", "Aucune"],
      answer: 0,
    },
    {
      id: 18,
      question: "Saisir des nombres jusqu'à obtenir 0 ",
      options: ["Pour", "Répéter Jusqu'à", "Cas", "Si"],
      answer: 1,
    },
    {
      id: 19,
      question: "Répétitions inconnues mais potentiellement nulles ",
      options: ["Pour", "Répéter", "Tant Que", "Si"],
      answer: 2,
    },
    {
      id: 20,
      question: "Répétitions inconnues mais au moins une exécution ",
      options: ["Pour", "Tant Que", "Répéter Jusqu'à", "Cas"],
      answer: 2,
    },
  ],
};
